...
import { getPopularVideo, testfxn } from './redux/slices/videoslices';
...
const App = () => {
    ...
    const getUser = () => {
      fetch("http://localhost:5000/auth/login/success", {
        ...
        throw new Error("authentication has been failed!")
          ...
          dispatch(addUser(resObject.user.displayName));
      }).catch((err) => {
        console.log(err);
      });
    };
  ...
  return (
    ...
    { (
      <BrowserRouter>
      <Routes> 
      <Route exact path='/' element={ <Layout homescreen={<HomeScreen />}/>  } />
